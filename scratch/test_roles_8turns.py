...
for role in ["core java", "python developer", "react frontend", "data engineer", "devops", "cloud"]:
    print(f"\n================ ROLE: {role} (max_turns=8) ================")
    req = CreateSessionRequest(mode="demo", role=role, max_turns=8)
    active = global_session_store.create_session(req)
    runner = active.runner
    
    q = runner.start()
    print(f"Turn 1: {q.id} -> {q.question_text[:50]}")
    
    for turn in range(1, 10):
        if runner.is_complete:
            print(f"Completed after {len(runner.state.question_history)} questions.")
            break
        res = runner.submit_answer("A comprehensive answer explaining the concepts and design trade-offs.")
        if res.next_question:
            print(f"Turn {runner.state.current_turn}: {res.next_question.id} -> {res.next_question.question_text[:50]}")
        else:
            print(f"Turn {runner.state.current_turn}: Completed. No next question.")
            
    # Check for any duplicates in question history
