...
export function AuthModal() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setErrorMessage("Google Sign-In failed. Please verify configuration.");
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setErrorMessage(res.error === "CredentialsSignin" ? "Sign-in failed. Please check your email." : res.error);
      } else {
        await refreshUser();
        closeAuthModal();
      }
        ...
        <form onSubmit={handleEmailSubmit} className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              Links your mock sessions, live credits and BYOK keys to your account.
            </p>
          </div>
          ...
          <button
            type="submit"
            disabled={loading || googleLoading}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-brand-600 hover:bg-brand-500 px-4 py-2.5 text-xs font-semibold text-white transition-all shadow-lg shadow-brand-500/20 disabled:opacity-50 cursor-pointer"
          >
            {loading ? (
          ...
          <div className="mt-5 border-t border-slate-800/80 pt-4 text-center">
            ...
            <div className="flex items-center justify-center gap-2">
              <button
                ...
                className="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-[11px] font-mono text-slate-300 transition-colors cursor-pointer"
              >
                candidate@kramix.ai
              </button>
              <button
